...
function stripLiteraryLanguage(text: string): string {
  let out = text;
...
function trimAdjectiveStacks(text: string): string {
  if (!text) return text;
...
function detectVoiceIssues(text: string, voice: "scene" | "witness" | "narrator") {
  const issues: string[] = [];
...
export function voiceGuard(text: string, voice: "scene" | "witness" | "narrator" = "witness") {
